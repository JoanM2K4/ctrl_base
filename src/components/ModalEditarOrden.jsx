...
function ModalEditarOrden({ orden, onActualizado }) {
  ...
  useEffect(() => {
    if (orden) {
      if (orden.estado === "Completada") {
        alert("❌ No puedes editar una orden completada.")
        document.getElementById("modal-editar-orden").checked = false
        return
      }
      setForm({
      ...
      fetch(`/api/ordenes/ver_orden.php?orden_id=${orden.orden_id}`)
        .then(res => res.json())
        .then(data => setCarrito(data.productos))
    }
    fetch("/api/proveedores/listar_proveedores.php").then(res => res.json()).then(setProveedores)
    fetch("/api/inventario/listar_inventario.php").then(res => res.json()).then(setProductos)
  }, [orden])
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch("/api/ordenes/editar_orden.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ orden_id: orden.orden_id, ...form, productos: carrito })
    })
    const data = await res.json()
    if (res.ok) {
      const toast = document.getElementById("notificaciones")
      toast.innerHTML = `<div class="alert alert-success"><span>✅ Orden actualizada</span></div>`
  ...
  return (
    <>
      <input type="checkbox" id="modal-editar-orden" className="modal-toggle" />
      <div className="modal">
        <div className="modal-box max-w-2xl">
          <h3 className="font-bold text-lg mb-4">Editar Orden de Compra</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between mt-4">
              ...
              <div className="modal-action">
                ...
                <label htmlFor="modal-editar-orden" className="btn">Cancelar</label>
              </div>
            </div>
          </form>
        </div>
